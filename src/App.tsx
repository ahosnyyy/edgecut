...
import { SparklesIcon, AlertCircleIcon, PackageIcon, Settings01Icon, CheckListIcon } from '@hugeicons/core-free-icons';
...
function AppContent() {
  ...
  const totalPieces = state.demandPieces.reduce((sum: number, p: any) => sum + p.quantity, 0);
  ...
  return (
    <div className="h-screen bg-background text-foreground flex flex-col overflow-hidden">
      ...
      {state.validationErrors.length > 0 && (
        <div className="px-4 pt-2 no-print w-full">
            ...
            <CardContent className="px-4 py-2 flex items-start gap-3">
              ...
              <div className="flex flex-col gap-0.5">
                {state.validationErrors.map((err: string, i: number) => (
                  <span key={i} className="text-sm text-destructive">{err}</span>
                ))}
              </div>
            </CardContent>
          </Card>
      ...
      <main className="flex-1 hidden lg:flex overflow-hidden">
        ...
        <aside className="no-print w-[280px] xl:w-[320px] shrink-0 flex flex-col border-r overflow-hidden">
          <Tabs defaultValue="desktop-stock" className="flex-1 flex flex-col overflow-hidden">
            <div className="px-4 pt-4 pb-2">
              <TabsList className="w-full grid grid-cols-2">
                <TabsTrigger value="desktop-stock" className="gap-1.5">
                  <HugeiconsIcon icon={PackageIcon} size={14} className="pointer-events-none" /> Stock
                </TabsTrigger>
                <TabsTrigger value="desktop-settings" className="gap-1.5">
                  <HugeiconsIcon icon={Settings01Icon} size={14} className="pointer-events-none" /> Settings
                </TabsTrigger>
              </TabsList>
            </div>

            <div className="flex-1 overflow-y-auto px-4 pb-4">
              <TabsContent value="desktop-stock" className="mt-0 focus-visible:ring-0 focus-visible:ring-offset-0">
                <StockPanel />
              </TabsContent>
              <TabsContent value="desktop-settings" className="mt-0 focus-visible:ring-0 focus-visible:ring-offset-0">
                <SettingsPanel />
              </TabsContent>
            </div>
          </Tabs>
        ...
        <aside className="no-print w-[320px] xl:w-[360px] shrink-0 flex flex-col border-l overflow-hidden">
          ...
          <div className="border-t bg-background/95 backdrop-blur px-4 py-3 shrink-0">
            <div className="flex items-center justify-between gap-3">
              ...
              >
                <HugeiconsIcon icon={SparklesIcon} size={16} className={`mr-1.5 ${state.optimizeAnimation ? 'animate-spin' : ''}`} />
                {state.optimizeAnimation ? 'Working...' : 'Optimize'}
              </Button>
              <span className="text-xs text-muted-foreground truncate">
                <strong className="text-foreground">{totalPieces}</strong> pcs ×{' '}
                <strong className="text-foreground">{state.stockLengths.length}</strong> stock
              </span>
            </div>
          </div>
        </aside>
      ...
      <main className="flex-1 flex flex-col lg:hidden overflow-hidden">
        <Tabs defaultValue="mobile-stock" className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-4 no-print">
            <TabsContent value="mobile-stock" className="mt-0 focus-visible:ring-0 focus-visible:ring-offset-0">
              <StockPanel />
            </TabsContent>
            <TabsContent value="mobile-settings" className="mt-0 focus-visible:ring-0 focus-visible:ring-offset-0">
              <SettingsPanel />
            </TabsContent>
            <TabsContent value="mobile-demand" className="mt-0 focus-visible:ring-0 focus-visible:ring-offset-0">
              <DemandPieces />
            </TabsContent>
          </div>

          {/* Pinned Optimize Bar (mobile) */}
          <div className="no-print border-t bg-background/95 backdrop-blur px-4 py-2.5">
            <div className="flex items-center justify-between gap-3">
              ...
              <Button
                ...
                disabled={state.optimizeAnimation || !hasInput}
              >
                <HugeiconsIcon icon={SparklesIcon} size={16} className={`mr-2 ${state.optimizeAnimation ? 'animate-spin' : ''}`} />
